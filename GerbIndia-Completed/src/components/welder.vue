<template>
  ...
  <div class="flex  flex-row justify-around  fixed top-16 mt-1 w-full bg-blue-100 ">
    ...
    <div class="flex space-x-4 p-4 ml-24 ">
      <span class="text-lg font-montserrat font-bold text-purple-500">Machines States </span> 
           ...
           <tbody  class="divide-y divide-gray-300">
             <tr v-for="row in dataRows" :key="row.id" class="border-t hover:bg-white transition-colors ">
               ...
               <td class="flex flex-col items-center sm:px-4 sm:py-16 font-poppins">
                  <div class="flex space-y-4 flex-col items-center font-poppins pt-16">
                    ...
                    <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center h-12">{{ row.plate }}</div>
                    <div v-else><input v-model="row.plate" class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center" placeholder="platediscription"></div>
                

                    <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center h-12">{{ row.platediscription }}</div>
               ...
               <td>
                 <div class=" flex space-y-4 flex-col rounded-md items-center font-poppins">
                  <div  v-if="!row.isEditing" class="text-center"><td class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mt-2 w-40 text-center h-12"><b>{{ row.element }}</b></td></div>
                  <div v-else>
                 ...
                 <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center h-12"><b> {{ row.element_description || 'N/A' }}</b></div>
                 ...
                 <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center h-12"><b> {{ row.range || 'N/A' }}</b></div>
...
const handleSubmit = async (machineId) => {
  try {
    ...
    const url = `http://172.18.100.33:6969/all_logs/${machineId}/${startDate.value}/${endDate.value}`;
...
onMounted(async () => {
  ...
  for (const row of dataRows.value) {
    try {
      ...
      const welderResponse = await fetch(`http://172.18.100.33:6969/welder/${mostRecentOperatorName}`);
...
;
      ...
      const elementResponse = await fetch(`http://172.18.100.33:6969/elements/${mostRecentElementType}`);
      const elementData = await elementResponse.json();
...
const connectToWebSocket = () => {
  const socket = new WebSocket('ws://172.18.100.33:6969/live_recent_ws');
...
async function saveEditedData(row) {
  try {
    ...
    const response = await axios.put(`http://172.18.100.33:6969/edit/${row.element}/${row.operator}?machine_id=${row.itemno}`, {
...
// console.log(element_description)
    ...
    const recentDataResponse = await axios.get(`http://172.18.100.33:6969/op_shift/${machineId}`);
...
;
      ...
      // console.log(mostRecentData.start_time);
       ...
       if (originaltype2 !== row.element) {
        const createEntryResponse = await axios.post("http://172.18.100.33:6969/op_shift/", {
...
function showUserPopup(machineId) {
  ...
  axios.get(`http://172.18.100.33:6969/live_data/live_type/${machineId}`)
    .then(response => {
...
//   try {
//     console.log(machineId)
//     const response = await axios.put(`http://172.18.100.33:6969/edit/${machineId.element}/${machineId.operator}`);
...
const fetchDatastate = async () => {
  try {
    const response = await axios.get('http://172.18.100.33:6969/machine_color/');
