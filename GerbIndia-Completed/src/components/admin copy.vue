<template>
  <div>
  <h3 class=" font-poppins text-purple-500 text-xl font-bold ml-96 bg-blue-50 absolute flex flex-col">Admin page</h3>
</div>
          ...
          <tbody>
            <tr v-for="row in dataRows" :key="row.id" class="border-t hover:bg-white transition-colors">
              ...
              <div class="flex space-x-2 mt-2 ml-24">
                  <button @click="toggleEdit(row)" v-if="!row.isEditing" class="glassmorphic-button bg-blue-500 text-white px-2 py-1 rounded-md w-10 h-8 mb-2"><i class="fa-solid fa-pen-to-square"></i> </button>
                  <button @click="saveEditedDataElement(row)" v-else class="glassmorphic-button bg-green-500 text-white px-2 py-1 rounded-md w-10 h-8 mb-2" ><i class="fa-regular fa-floppy-disk" ></i> </button>
                </div>
              </td>
              <!-- <td>
                <div
              ...
              <td>
                ...
                <div class=" flex space-y-2 flex-col items-center  sm:px-4 sm:py-16 font-poppins">
                  ...
                  <div class="flex space-x-2 mt-2">
                  <button @click="toggleEdit(row)" v-if="!row.isEditing" class="glassmorphic-button bg-blue-500 text-white px-2 py-1 rounded-md w-10 h-8 mb-2"><i class="fa-solid fa-pen-to-square"></i> </button>
                  <button @click="saveEditedData(row)" v-else class="glassmorphic-button bg-green-500 text-white px-2 py-1 rounded-md w-10 h-8 mb-2" ><i class="fa-regular fa-floppy-disk" ></i> </button>
                </div>
                </div>
              </td>
  ...
  async function saveEditedData(row) {
    try {
      ...
      updateoperatorData(row.id, row.I_no, row.Fc_no, row.project, row.current,row.voltage, row.element_description);
  ...
  async function saveEditedDataElement(row) {
    try {
      const response = await axios.put(`http://172.18.7.66:6060/edit/${row.element}`, {
        current: row.current, // Use the actual current value from the row
        voltage: row.voltage, // Use the actual voltage value from the row
        element_description: row.element_description,
      });
