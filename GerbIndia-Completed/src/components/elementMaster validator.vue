...
<button @click="showElementForm"
  ...
  <div v-if="alertMessage" class="fixed inset-0 flex items-center justify-center z-50">
    <div class="w-96 p-4 bg-gray-100 border border-gray-300 rounded-lg shadow-md">
      ...
      <button @click="closeAlert" class="bg-blue-500 text-white px-2 py-1 rounded-lg mt-2 "><p class="text-center">OK</p></button>
    </div>
  </div>
</template>
...
const submitElementForm = async () => {
  try {
    ...
    alertMessage.value = 'Element created successfully';
    ...
    if (error.response && error.response.status === 422) {
      // If the server responds with a validation error (422 Unprocessable Entity),
      // it means the element type already exists
      alertMessage.value = 'Error creating element: Type already exists';
    } else {
      // Handle other errors
      alertMessage.value = 'Error creating element: ' + error.message;
    }
  }
};
