<template>
<!-- <Navbar/> -->
<div class="flex  flex-row justify-around  fixed top-16 mt-1 w-full bg-blue-100 ">
<h3 class="font-poppins text-purple-500 text-xl font-bold mt-3 mr-24">WELDER PAGE</h3>
<div class="flex space-x-4 p-4 ml-24 ">
  <span class="text-lg font-montserrat font-bold text-purple-500">Machines Status </span> 
...
<div class="card p-8 bg-blue-50 shadow-lg rounded-lg ">
  <div class="overflow-auto h-full">
    ...
    <table class="w-full" >
      <thead class ="" >
        <!-- <tr class="bg-blue-500 text-white fixed top-32 w-full">
           <th class="px-2 py-5 font-poppins ">Item No</th>
           <th class="px-5 py-2 font-poppins">Welding Machine</th>
           <th class="px-28  py-2 font-poppins">Operator</th>
           <th class="px-28 py-2 font-poppins">Plate</th>
           <th class="px-6 py-2 font-poppins">Element</th>
           <th class="px-20 py-2 font-poppins">STANDARD</th>
           <th class="px-1 py-2 font-poppins ">ACTUAL</th>
           <th class="px-16 py-2 font-poppins">Live</th>
           <th class="px-16 py-2 font-poppins">Project</th>
           <th class="px-24 py-2 font-poppins">Remarks</th>
         </tr> -->
         <tr class="bg-blue-500 text-white fixed top-32 w-full">
           <th class="px-2 py-5 font-poppins ">Item No</th>
           <th class="px-5 py-2 font-poppins">Welding Machine</th>
           ...
           <th class="px-6 py-2 font-poppins">Element</th>
           <th class="px-20 py-2 font-poppins">STANDARD</th>
           <th class="px-5 py-2 font-poppins ">ACTUAL</th>
           <th class="px-12 py-2 font-poppins">Live</th>
           <th class="px-20 py-2 font-poppins">Project</th>
           <th class="px-28 py-2 font-poppins">Remarks</th>
         </tr>
       </thead> 
       ...
       <tbody  class="divide-y divide-gray-300">
         <tr v-for="row in dataRows" :key="row.id" class="border-t hover:bg-white transition-colors ">
           ...
           <td>
            <!-- <div class=" flex space-y-2 flex-col items-center  sm:px-4 sm:py-16 font-poppins">
             ...
             <div class="border border-gray-300 flex space-y-2 flex-col items-center  sm:px-4 sm:py-16 font-poppins">
               ...
               <td> <div class="text-lg font-bold text-center"> {{ row.operator }}</div><br>
                ...
                <div><span class="font-bold">Start Time: </span><br>{{ formatDateTime(row.startTime) }}</div><br>
                <div><span class="font-bold">End Time: </span><br>{{ formatDateTime(row.endTime) }}</div>
           ...
           <td class="flex flex-col items-center sm:px-4 sm:py-16 font-poppins">
              <div class="flex space-y-4 flex-col items-center font-poppins pt-32">
                ...
                <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 text-center w-40 h-auto my-2 overflow-hidden break-words" :style="{ minHeight: '2rem' }">{{ row.plate }}</div>
                <div v-else><input v-model="row.plate" class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center h-36" placeholder="plate"></div>
            

                <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 text-center w-40 h-auto my-2 overflow-hidden break-words" :style="{ minHeight: '2rem' }">{{ row.platediscription }}</div>
                <div v-else><input v-model="row.platediscription" class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center" placeholder="platediscription"></div>
              </div>
            </td>
           ...
           <td>
             <div class=" flex space-y-4 flex-col rounded-md items-center font-poppins">
              <!-- <div class="text-lg font-bold text-center -mb-4"> {{  row.element }}</div><br> -->
              <div class="flex justify-center mb-4">
                <div class="text-lg font-bold text-center w-40 overflow-hidden break-words">
                  {{ row.element }}
                </div>
              </div>

              <!-- <div class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mt-2 w-40 text-center h-12 -mb-4"> {{  row.element }}</div><br> -->
              <!-- <div  v-if="!row.isEditing" class="text-center"><td class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mt-2 w-40 text-center h-12" disabled><b>{{ row.element }}</b></td></div>
              <div v-else> -->
               <!-- <select v-model="row.element" @change="onOptionSelected($event)"
                 class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center ">
               ...
               <!-- <select v-model="row.element" required class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center"  @change="onOptionSelected($event)" @click="fetchElementTypes" disabled>
        ...
        <option v-for="elementType in elementTypes" :key="elementType">{{ elementType }}</option>
             ...
             <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 text-center w-40 h-auto my-2 overflow-hidden break-words"><b> {{ row.element_description || 'N/A' }}</b></div>
             <div v-else><input v-model="row.element_description" class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center" placeholder="Discription"></div>

             


             <!-- <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center"><b> {{ row.element_description }}</b></div>
             <div v-else><input v-model="row.element_description" class="border border-gray-300 px-4 py-2 rounded-md w-40 text-center" placeholder="elementDescription"></div> -->
           

             <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 text-center w-40 h-auto my-2 overflow-hidden break-words"><b> {{ row.range || 'N/A' }}</b></div>
           ...
           <td>
             <div class="flex space-y-2 flex-col items-center mb-3 mt-3 font-poppins">
               <div class="border border-gray-300 px-4 py-6 rounded-md h-25 w-36">
                 ...
                 <div v-if="!row.isEditing" type="text"  class="border border-gray-300 rounded-md ml-4 h-auto w-24 my-2 overflow-hidden break-words">
                   <p class="text-lg text-blue-800 text-center font-bold  "> {{ row.standard.standard_current }}A</p></div>
                 <div v-else><input v-model=" row.standard.standard_current" class="border border-gray-300 px-4 py-2 rounded-md ml-4 h-8 w-24 text-center" placeholder="current range" disabled></div>
               </div>
               <div class="border border-gray-300 px-4 py-6 rounded-md h-25 w-36 ">
                 ...
                 <div v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md ml-4 h-auto w-24 my-2 overflow-hidden break-words">
                   ...
                   <div v-else><input v-model="row.standard.standard_voltage" class="border border-gray-300 px-4 py-2 rounded-md ml-4 h-8 w-24 text-center" placeholder="voltage range" disabled></div>
                 </div>
             </div>
           </td>
           <td>
...
<div class="border border-gray-300 px-4 py-6 rounded-md h-25 w-36 ">
  ...
  <div class="border border-gray-300 rounded-md ml-4 h-auto w-24 my-2 overflow-hidden break-words">
    <p class="text-lg text-center font-bold text-lime-500">{{ getMachineLiveCurrent(row.id) }}A</p>
  </div>
</div>
<div class="border border-gray-300 px-4 py-6 rounded-md h-25 w-36">
...
Loading data...
           ...
           <td>
             ...
             <!-- I No Section -->
    <div class="border border-gray-300 px-4 py-6 rounded-md w-36">
      <p class="text-lg text-black text-center font-bold -mt-4">I No</p>
      <div v-if="!row.isEditing" class="border border-gray-300 rounded-md ml-3 h-auto w-24 my-2 overflow-hidden break-words" :style="{ minHeight: '2rem' }">
        <p class="text-lg text-blue-800 text-center font-bold">{{ row.I_no}}</p>
      </div>
      <div v-else>
        <input v-model="row.I_no" class="border border-gray-300 px-4 py-2 rounded-md ml-4 h-8 w-24 text-center" placeholder="I No">
      </div>
    </div>
    ...
    <div class="border border-gray-300 px-4 py-6 rounded-md w-36">
      ...
      <div v-if="!row.isEditing" class="border border-gray-300 rounded-md ml-3 h-auto w-24 my-2 overflow-hidden break-words" :style="{ minHeight: '2rem' }">
        <p class="text-lg text-blue-800 text-center font-bold">{{ row.Fc_no }}</p>
      </div>
      <div v-else>
        <input v-model="row.Fc_no" class="border border-gray-300 px-4 py-2 rounded-md ml-4 h-8 w-24 text-center" placeholder="Fc No">
      </div>
    </div>
    ...
    <div class="border border-gray-300 px-4 py-6 rounded-md w-36">
      ...
      <div v-if="!row.isEditing" class="border border-gray-300 rounded-md h-auto w-28 my-2 overflow-hidden break-words" :style="{ minHeight: '2rem' }">
        <p class="text-lg text-blue-800 text-center font-bold">{{ row.project }}</p>
      </div>
      <div v-else>
        <input v-model="row.project" class="border border-gray-300 px-4 py-2 rounded-md h-8 w-28 text-center" placeholder="Project">
      </div>
    </div>
           ...
           <div v-if="isStackVisible" class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <!-- 2. Chart Display Section -->
              <div class="m-2">
                ...
                <!-- 4. Close Button -->
                <button @click="hideStackPopup" class="bg-white px-1 py-0 rounded-lg absolute top-2 right-0 -mt-1 mr-3">
                    <!-- 5. Close Icon -->
                    <img width="20" height="20" src="https://img.icons8.com/ios-glyphs/30/delete-sign.png" alt="delete-sign" />
                </button>
             </div>
...
const handleScroll = () => {
if (tableRef.value) {
const sticky = tableRef.value.offsetTop;
isFixedHeader.value = window.pageYOffset > sticky;
}
};
...
try {
  ...
  console.log("🚀 ~ onMounted ~ mostRecentElement:", mostRecentElement)
  ...
  console.log("🚀 ~ onMounted ~ mostRecentElementType:", mostRecentElementType)
  ...
  row.element_type = mostRecentElementType;
  // console.log("))))))))))))))))))))))))))")
...
// `http://172.18.100.54:6969/elements/${mostRecentElementType}`
  ...
  // Assuming the response contains only one item
  const { standard_current, standard_voltage, range,element_description } = elementData;
  ...
  row.range = range;
  row.element_description = element_description;
...
// const editedType = response.data.element;
// console.log("78999999999999998")
// console.log(editedType)
alertMessage.value = `Edited Data ${row.itemno} successfully`;
//     console.log("************&&&&&&&&&&&&&&&&&&&")
...
if (operatorRecentData.length > 0) {
  ...
  const originaltype2= mostRecentData.element_type;
  ...
  // console.log(mostRecentData.start_time);
   ...
   // Check if the type has changed from the original value
  ...
  //  console.log(originaltype2)
   if (originaltype2 !== row.element) {
    ...
    console.log("Type not changed. No new entry created.");
  }
} else {
...
// 1. Set a reactive variable to make the user live current popup visible
isUserLiveCurrentPopupVisible.value = true;

// 2. Set the machineId for graph data
graphData.value.machineId = machineId;
...
.then(response => {
  ...
  if (response.data && response.data.length > 0) {
    // 5. Extract values from the response data
    const lowStdVtgValue = response.data[0].low_std_vol;
    const highStdVtgValue = response.data[0].high_std_vol;
    const lowStdcurValue = response.data[0].low_std_curr;
    const highStdcurValue = response.data[0].high_std_curr;

    // 6. Update graphData with range values
    graphData.value.range_voltage_low = lowStdVtgValue;
    graphData.value.range_voltage_high = highStdVtgValue;
    graphData.value.range_current_low = lowStdcurValue;
    graphData.value.range_current_high = highStdcurValue;

    // 7. Log low standard voltage value to the console
    console.log('Low Standard Voltage:', lowStdVtgValue);
  } else {
    // 8. Log an error if there is no data or the array is empty
    console.error('No data or empty array in the response');
  }
  // 9. Assuming sendata is defined somewhere in your component,
  //    call the sendata function to update range values
  //    sendata(machineId);
})
.catch(error => {
  // 10. Log an error if there is an issue fetching data
  console.error('Error fetching data:', error);
});
}
