...
;
      ...
      const elementResponse = await fetch(`http://172.18.100.54:6969/elements/${mostRecentElementType}`);
      const elementData = await elementResponse.json();
...
// console.log("************************************")
  ...
  //    try {
  //     for (const row of dataRows.value){
  //      const response = await axios.get(`http://172.18.100.54:6969/elements/${row.element}`);
...
async function saveEditedData(row) {
  try {
    ...
    const response = await axios.put(`http://172.18.100.54:6969/edit/${row.element}/${row.operator}?machine_id=${row.itemno}`, {
