...
<!-- Add a filter button -->
<button @click="showFilterForm" class="bg-gray-500 glassmorphic-button rounded-lg px-4 py-2 mt-2 mb-2 ml-3 text-white font-poppins flex flex-wrap">
  <img width="24" height="24" src="https://img.icons8.com/ios-filled/50/FFFFFF/filter-and-sort.png" alt="filter-and-sort" class="w-6 h-6 mr-2"/>Filter Table
</button>
...
<!-- <button @click="downloadTableData"
        ...
        <button @click="downloadTableDataExcel" class="bg-green-500 glassmorphic-button rounded-lg px-4 py-2 mt-2 mb-2 ml-3 text-white font-poppins flex flex-wrap">
        <img width="24" height="24" src="https://img.icons8.com/sf-black/64/FFFFFF/download.png" alt="download" class="flex flex-wrap"/>
         Download Table Excel
        </button>
        <div class="flex">
      <button @click="showProductionForm" class="bg-purple-500 glassmorphic-button rounded-lg px-4 py-2 mt-2 mb-2 ml-3 text-white font-poppins flex flex-wrap">
        <img src="https://img.icons8.com/material-outlined/24/FFFFFF/add.png" alt="add" class="w-6 h-6 mr-2" />
        Download Production Data
      </button>
    </div>
        <button @click="resetFilters" class="bg-gray-700 glassmorphic-button rounded-lg px-4 py-2 mt-2 mb-2 ml-3 text-white font-poppins flex flex-wrap">
          <img width="24" height="24" src="https://img.icons8.com/ios-filled/50/FFFFFF/connection-sync.png" alt="connection-sync" class="w-6 h-6 mr-2"/>  Reset Filters</button>
    ...
    <div class="container overflow-x-auto min-w-full" ref="reportContainerTable">
      <h1 class="text-2xl font-semibold my-4">Machine Data</h1>
      <table class="table-auto border-collapse min-w-full">
        <thead>
           <tr>
    <th v-for="(header, index) in tableHeaders" :key="index" class="px-2  py-2">{{ header }}</th>
    <th class=" px-2 py-2">Actions</th>
        ...
        <tbody>
          <tr v-for="(data, index) in paginatedData" :key="index">
            ...
            <td class="border px-4 py-2">{{ formatDateTime(data.start_time) }}</td>
        <td class="border px-4 py-2">{{ formatDateTime(data.end_time) }}</td>
            <!--<td class="border px-4 py-2">{{ data.shift }}</td>-->
            <td class="border px-4 py-2">{{ data.I_no }}</td>
            <td class="border px-4 py-2">{{ data.Fc_no }}</td>
            <td class="border px-4 py-2">{{ data.project }}</td>
            <td class="border px-4 py-2">{{ data.plate_thickness }}</td>
            <td class="border px-4 py-2">{{ data.plate_description }}</td>
            <td class="border px-4 py-2">{{ data.remarks }}</td>
            <td class="border flex flex-row px-4 py-2">
              <button @click="editMachine(index)" class="bg-blue-500 glassmorphic-button text-white px-2 py-1 rounded-lg mr-2">Edit</button>
              <button @click="deleteData(data.machine_name, data.start_time, data.end_time, data.element_type, data.operator_name)" class="bg-red-500 glassmorphic-button text-white font-bold py-2 px-4 rounded">
               Delete
              </button>
      ...
      <div v-if="isFormVisible" class="fixed inset-0 flex items-center justify-center z-50">
        <div class="w-96 p-4 bg-gray-100 border border-gray-300 rounded-lg shadow-md">
          <h2 class="text-lg font-semibold text-gray-800">{{ isSaveMode ? 'Edit Machine' : 'Create Schedule List' }}</h2>
    ...
    <select v-model="formData.operator_name" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchOperators">
      ...
      <select v-model="formData.I_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchI_noValues">
        <option :value="null">Select I_no</option>
        <option v-for="I_noValue in I_noValues" :key="I_noValue">{{ I_noValue }}</option>
      </select>
    </div>
    <div class="mb-2">
      <label class="block text-gray-800">Fc_no:</label>
      <select v-model="formData.Fc_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchFc_noValues">
        <option :value="null">Select Fc_no</option>
        <option v-for="Fc_noValue in Fc_noValues" :key="Fc_noValue">{{ Fc_noValue }}</option>
      </select>
    </div>
    <div class="mb-2">
      <label class="block text-gray-800">Project:</label>
      <select v-model="formData.project" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchProjectValues">
        <option :value="null">Select Project</option>
        <option v-for="projectValue in projectValues" :key="projectValue">{{ projectValue }}</option>
      </select>
    </div> -->
    ...
    <div class="mb-2">
    <label class="block text-gray-800">I_no:</label>
    <input v-model="formData.I_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
  </div>
  <div class="mb-2">
    <label class="block text-gray-800">Fc_no:</label>
    <input v-model="formData.Fc_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
  </div>
  <div class="mb-2">
    <label class="block text-gray-800">Project:</label>
    <input v-model="formData.project" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
  </div>
      ...
      <div v-if="isProductionFormVisible" class="fixed inset-0 flex items-center justify-center z-50">
        <div class="w-96 p-4 bg-gray-100 border border-gray-300 rounded-lg shadow-md">
          <form @submit.prevent="generateProductionExcel($event)">
            <div class="mb-2">
              <label class="block text-gray-800">Machine Name:</label>
              <select v-model="machine_id" class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchMachineNames">
                <option value="">Select Machine Name</option>
                <option v-for="machineId in machineIds" :key="machineId">{{ machineId }}</option>
              </select>
            </div>
            ...
            <div class="mb-2">
              ...
              <input v-model="startDateProd" type="date"  class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
            </div>

            <div class="mb-2">
              <label class="block text-gray-800">End Date:</label>
              <input v-model="endDateProdProd" type="date"  class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
            </div>
            <div class="mt-2 flex justify-end">
        <button  type="submit" class="bg-blue-500 text-white px-2 py-1 rounded-lg mr-2">Download</button>
        <button @click="cancelProductionForm" class="bg-gray-300 text-gray-700 px-2 py-1 rounded-lg">Cancel</button>
      </div>
          </form>
        </div>
      </div>
  ...
  <div class="mb-2">
    ...
    <label class="block text-gray-800">I_no:</label>
    <select v-model="formData.I_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchI_noValues">
      <option value="">Select I_no</option>
      <option v-for="I_noValue in I_noValues" :key="I_noValue">{{ I_noValue }}</option>
    </select>
  </div>
  <div class="mb-2">
    <label class="block text-gray-800">Fc_no:</label>
    <select v-model="formData.Fc_no" required class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchFc_noValues">
      <option value="">Select Fc_no</option>
      <option v-for="Fc_noValue in Fc_noValues" :key="Fc_noValue">{{ Fc_noValue }}</option>
    </select>
  </div>
  ...
  <div class="w-96 p-4 bg-gray-100 border border-gray-300 rounded-lg shadow-md">
    ...
    <form @submit.prevent="applyFilters">
      <div class="mb-2">
    <label class="block text-gray-800">operator_name:</label>
    <select v-model="formData.operator_name" @change="updateFilterData" class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchOperators">
      <option value="">Select Operator Type</option>
      <option v-for="operator in operators" :key="operator" >{{ operator }} </option>
    </select>
  </div>
      <div class="mb-2">
        ...
        <select v-model="filterData.machineId" @change="updateFilterData" class="border border-gray-300 rounded-lg px-2 py-1 w-full">
          <option value="">Select Machine Name</option>
          <option v-for="machineId in machineIds" :key="machineId">{{ machineId }}</option>
        </select>
      </div>
      <div class="mb-2">
        <label class="block text-gray-800">Start Date:</label>
        <input v-model="filterData.startDate" @change="updateFilterData" type="date" class="border border-gray-300 rounded-lg px-2 py-1 w-full" />
      </div>
      <div class="mt-2 flex justify-end">
...
<div v-if="isFilterFormVisible" class="fixed inset-0 flex items-center justify-center z-50">
    <div class="w-96 p-4 bg-gray-100 border border-gray-300 rounded-lg shadow-md">
      ...
      <form @submit.prevent="applyFilters">
        <div class="mb-2">
          <label class="block text-gray-800">operator_name:</label>
          <select v-model="filterData.operator_name" class="border border-gray-300 rounded-lg px-2 py-1 w-full" @click="fetchOperators">
            <option value="">Select Operator Type</option>
            <option v-for="operator in operators" :key="operator">{{ operator }}</option>
          </select>
        </div>
...
// Compute total number of pages based on data length
...
// Compute paginated table data based on currentPage and pageSize
...
const submitWelderForm = async () => {
  try {
    ...
    // Make the HTTP POST request to your FastAPI backend
    const response = await axios.post('http://172.18.100.54:6969/welder/', formData);
...
const axiosOpShiftUrl = computed(() => {
  return `http://172.18.100.54:6969/op_shift/`;
});
...
//to fetch all the details in the table
const fetchMachineNames = async () => {
  const machinesUrl = 'http://172.18.100.54:6969/machines'; // Replace with the actual endpoint
  try {
...
onMounted(() => {
  fetchElementTypes(); // Fetch machine IDs when the component is mounted
});
const operators = ref([]); // Store machine IDs

const fetchOperators = async () => {
  const machinesUrl = 'http://172.18.100.54:6969/welder/'; // Replace with the actual endpoint
  ...
  try {
    ...
    if (Array.isArray(response.data)) {
      operators.value = response.data.map((machine) => machine.welder_name);
...
const fetchShiftTypes = async () => {
  const machinesUrl = 'http://172.18.100.54:6969/shift'; // Replace with the actual endpoint
  try {
    const response = await axios.get(machinesUrl);
...
const I_no = ref(null);
const Fc_no = ref(null);
const Project = ref(null);

const I_noValues = ref([]); // Store I_no values

const fetchI_noValues = async () => {
  const url = 'http://172.18.100.54:6969/project/';
  try {
    const response = await axios.get(url);
    I_noValues.value = response.data.Data.map((item) => item.I_no);
  } catch (error) {
    console.error('Error fetching I_no values:', error);
  }
};

const Fc_noValues = ref([]); // Store Fc_no values

const fetchFc_noValues = async () => {
  const url = 'http://172.18.100.54:6969/project/';
  try {
    const response = await axios.get(url);
    Fc_noValues.value = response.data.Data.map((item) => item.Fc_no);
  } catch (error) {
    console.error('Error fetching Fc_no values:', error);
  }
};

const projectValues = ref([]); // Store project values
const fetchProjectValues = async () => {
  const url = 'http://172.18.100.54:6969/project/';
  try {
    const response = await axios.get(url);
    projectValues.value = response.data.Data.map((item) => item.project);
  } catch (error) {
...
//     const url = `http://172.18.100.54:6969/op_shift/${machineId}`;
...
// Function to fetch and display the data for all machines
const fetchAndDisplayDataForAllMachines = async () => {
  try {
    // Fetch data from the endpoint for all machines
    const url = 'http://172.18.100.54:6969/op_shift/';
    const response = await axios.get(url);

    // Check if the response contains data
    if (response.data && response.data.length > 0) {
      // Set table headers based on the first item in the response
      tableHeaders.value = Object.keys(response.data[0]);

      // Append the data to existing tableData and originalTableData
      tableData.value = [...tableData.value, ...response.data];
      originalTableData.value = [...originalTableData.value, ...response.data];
    } else {
...
const deleteData = (machineName, startTime, endTime, elementName, operatorName) => {
  ...
  const url = `http://172.18.100.54:6969/op_shift/?machine_name=${machineName}&start_time=${startTimeEpoch}&end_time=${endTimeEpoch}&element_name=${encodedElementName}&operator_name=${encodedOperatorName}`;

  axios
    .delete(url)
    .then(() => {
      console.log(`Data for machine ${machineName} deleted successfully.`);
      alertMessage.value = `Data for machine ${machineName} deleted successfully.`;
...
//       await axios.post('http://172.18.100.54:6969/op_shift/', newMachine); // Updated URL
...
const fetchMachineData = async () => {
  ...
  const url = `http://172.18.100.54:6969/op_shift/machine-data?machineName=${formData.machineName}&operatorName=${formData.operator_name}`;
  const response = await axios.get(url);
...
const saveMachine = async () => {
  if (isSaveMode.value) {
    ...
    try {
      ...
      const url = `http://172.18.100.54:6969/op_shift/shiftops/update?machine_id=${formData.machineName}&operator_name=${formData.operator_name}`;
      alertMessage.value = `Machine ${formData.machineName} Edited successfully`;
...
setTimeout(() => {
    ...
    alertMessage.value = `Machine ${formData.machineName} saved successfully`;

    // Update the table data and reset the form
    tableData.value.push(newMachineString);
    resetFormData();
    isFormVisible.value = false;
    ...
    if (error.response && error.response.data && error.response.data.detail) {
      ...
      alertMessage.value = 'An error occurred while saving the machine.';
    }
  }
...
const editMachine = async (index) => {
  ...
  formData.I_no = selectedRow.I_no;
  formData.Fc_no = selectedRow.Fc_no;
  formData.project = selectedRow.project;
...
const updateFilterData = () => {
  filterData.operator_name = formData.operator_name;
  filterData.machineId = filterData.machineId;
...
//    const operatorNameMatch = !filterData.operator_name || data.operator_name === filterData.operator_name;
...
//     const startDateMatch = !filterData.startDate || 
//     (moment(filterData.startDate).isBetween(moment(data.start_time), moment(data.end_time), 'day', '[]'));
...
const applyFilters = async () => {
  try {
    const response = await axios.get('http://172.18.100.54:6969/scheduling_filter/shift_ops', {
      params: {
        machine_name: filterData.value.machineId,
        welder_name: filterData.value.operator_name,
        specific_date: filterData.value.startDate
      }
    });
...
// onMounted(async () => {
//   await fetchAndDisplayDataForAllMachines();
//   await fetchOperators();
...
const generateProductionExcel = async () => {
  if (!machine_id.value || !startDateProd.value || !endDateProdProd.value) {
    alert('Please fill in all fields');
    return;
  }

  const formattedStartDateProd = moment(startDateProd.value).format('YYYY-MM-DD');
  const formattedEndDateProdProd = moment(endDateProdProd.value).format('YYYY-MM-DD');

  const backendURL = `http://172.18.100.54:6969/graph/get_production_end_data?machine_id=${machine_id.value}&start_date=${formattedStartDateProd}&end_date=${formattedEndDateProdProd}`;

  try {
    const response = await axios.get(backendURL);
    console.log('Download successfully:', response.data);
    alertMessage.value = `${machine_id.value} Production Data Download successfully`;
    const productionData = response.data.map(item => ({
    ...
    const fileName = `productionExcel_${machine_id.value}.xlsx`;

    const ws = XLSX.utils.json_to_sheet(productionData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'ProductionDataSheet');
    ...
    if (error.response && error.response.data && error.response.data.detail) {
      // Specific detail from the backend
      alertMessage.value = error.response.data.detail;
    } else if (error.response) {
      // Handle different HTTP status codes
      if (error.response.status === 400) {
        alertMessage.value = `Bad Request: ${error.response.data.detail}`;
        ...
        if (errorMessage.includes("Element with type")) {
          ...
          alertMessage.value = `Validation Error: ${errorMessage}`;
        }
      } else {
        // Add more cases for other HTTP status codes if needed
        alertMessage.value = `An error occurred: ${error.response.data.message}`;
      }
    } else {
      // Handle other errors
